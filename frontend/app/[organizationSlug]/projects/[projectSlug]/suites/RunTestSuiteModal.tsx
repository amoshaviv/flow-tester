...
interface RunTestSuiteModalProps {
  ...
  onTestSuiteChange: (testSuiteSlug: string) => void;
...
export default function RunTestSuiteModal({
  isOpen,
  onClose,
  testSuites,
  selectedTestSuite,
  ...
  onRunTestSuite,
  isRunning,
}: RunTestSuiteModalProps) {
  const selectedTestSuiteObj = testSuites.find(suite => suite.slug === selectedTestSuite);
  const versions = selectedTestSuiteObj?.versions || [];
  const selectedVersionObj = selectedTestSuiteObj?.versions.find(version => version.slug === selectedVersion);
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <Typography variant="button">Run Test Suite</Typography>
            </Box>
            <Box sx={{ mr: 1 }}>
              <IconButton
                color="inherit"
            ...
            <Grid container spacing={2} sx={{ p: 1.5 }}>
              <Grid size={6}>
                <FormControl fullWidth>
                  <InputLabel id="test-suite-select-label">Select Test Suite</InputLabel>
                  <Select
                    labelId="test-suite-select-label"
                  ...
                  >
                    {testSuites.map((testSuite) => (
                      <MenuItem key={testSuite.slug} value={testSuite.slug}>
                        {testSuite.title || testSuite.defaultVersion?.title || `Test Suite ${testSuite.slug}`}
                      </MenuItem>
                    ))}
                  </Select>
                </FormControl>
                <FormControl fullWidth sx={{ mt: 1.6 }}>
                  ...
                  <Select
                    ...
                    disabled={isRunning || !selectedTestSuite}
                  >
          ...
          >
            ...
            <Button
              ...
              disabled={isRunning || !selectedTestSuite || !selectedVersion}
            >
              {isRunning ? (
