...
interface AddNewTestSuiteModalProps {
  onTestSuiteCreated?: () => void;
}

export default function AddNewTestSuiteModal({ onTestSuiteCreated }: AddNewTestSuiteModalProps) {
  const params = useParams();
  const { projectSlug, organizationSlug } = params;

  const [isSubmitting, setIsSubmitting] = useState<boolean>(false);
  const [isOpen, setIsOpen] = useState(false);
  const [createTestSuiteError, setCreateTestSuiteError] = useState<string | null>("");
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    try {
      ...
      await fetch(
        ...
        .catch((e) => {
          setIsSubmitting(false);
          setCreateTestSuiteError("An error occurred, please try again");
        });
    } catch {
      setCreateTestSuiteError("An error occurred, please try again");
    }
  ...
  return (
    <React.Fragment>
      <Button
        sx={{ borderRadius: "25px", width: "280px" }}
        size="large"
        variant="contained"
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                <Typography variant="button">Add new test suite</Typography>
              </Box>
              <Box sx={{ mr: 1 }}>
                <IconButton
                  color="inherit"
                  ...
                  <Grid size={12}>
                    <FormControl fullWidth>
                      <TextField
                        ...
                        name="description"
                        label="Description"
                        placeholder="A suite of tests covering user authentication, login, logout, and password management scenarios."
                        minRows={6}
                        maxRows={14}
                        multiline
                        variant="outlined"
